    ...
    private int AlphaBeta(ref Board board, int depth, int alpha, int beta, int ply, bool isPvNode)
    {
        try
        {
            ...
            for (int i = 0; i < moves.Count; i++)
            {
                ...
                if (_hashHistoryCount < _hashHistory.Length - 1)
                {
                    _hashHistory[_hashHistoryCount++] = newBoard.GetZobristHash();
                }
                else
                {
                    // If we've reached the limit, we can't track more positions
                    // This is extremely rare but prevents crashes
                    // In this case, we continue without tracking repetitions
                    throw new Exception("GET WRECKED");
                }
                int score;
